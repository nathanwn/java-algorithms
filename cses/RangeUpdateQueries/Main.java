...
public class Main {
    static void solve(InputReader in, PrintWriter out) {
        ...
        LazyIntervalTree it = new LazyIntervalTree(a, 0);
        for (int j = 0; j < q; j++) {
            int t = in.nextInt();
            if (t == 1) {
                ...
                int to = in.nextInt() - 1;
                int val = in.nextInt();
                it.update(from, to, val);
            } else {
                int pos = in.nextInt() - 1;
                out.println(it.query(pos, pos));
            }
        }
    }

    static class LazyIntervalTree {
        int n;
        long[] t;
        long[] z;
        boolean[] lazy;
        ...
        void updateNode(int node, long val) {
            t[node] += val;
            z[node] += val;
        }
        ...
        LazyIntervalTree(int[] a, long defaultValue) {
            this(a.length, defaultValue);
            build(a);
        }

        LazyIntervalTree(int len, long defaultValue) {
