...
public class Slow {
    static void solve(InputReader in, PrintWriter out) {
        ...
        for (int i = 0; i < n; i++) {
            yMin = Math.min(yMin, segments[i][1]);
            yMin = Math.min(yMin, segments[i][3]);
            yMax = Math.max(yMax, segments[i][1]);
            yMax = Math.max(yMax, segments[i][3]);
        }
        int dy = Math.max(0, -yMin);
        yMax += dy;
        for (int i = 0; i < n; i++) {
            segments[i][1] += dy;
            segments[i][3] += dy;
        ...
        IntervalTree it = new IntervalTree(yMax + 7);
        long ans = 0;
        for (int i = 0; i < events.size() - 1; i++) {
            ...
            ans += 1L * (next.x - cur.x) * it.yLength[1];
        }
        out.println(ans);
    }
